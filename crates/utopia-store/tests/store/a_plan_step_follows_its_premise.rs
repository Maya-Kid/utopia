...
//! 随后按唯一性方向对账——与人点头一条记忆事实（`pending::confirm`）同一条路。后面几个
//! 从开放陈述起步，经人定的短语绑定与 `materialize` 进类型化图谱，量的是对齐那一段：
//! 标了 `ignore` 的是期望语义今天还不成立的地方，`--ignored` 跑出来的失败就是最小复现。
//!
...
/// **缺口**：对齐物化出来的类型化行不走时间线。第二个位置落进类型化图谱之后，
/// 第一个位置照旧开着，读 `location` 的步骤照旧成立——直到有人显式对账。
/// 其它写类型化行的路（点头一条记忆、合并、改区间、删文档）都会对账
#[tokio::test]
...
/// **缺口**：同一个身份推新版本（一个东西一个 `external_id`）。旧陈述成了 stale 却没作废，
/// 照旧给类型化行作来源；两条类型化行的证据指着同一份文档，排序用的日期是那份文档
/// **当前**的 `doc_time`——两个值同一刻，显式对账也只记一对 simultaneous 冲突、不关任何一边
#[tokio::test]
#[ignore = "#875 gap: a same-identity update leaves both places open even after reconcile"]
async fn a_same_identity_update_closes_the_earlier_place() -> anyhow::Result<()> {
    let Some(url) = utopia_store::test_db::url() else {
        return Ok(());
    };
    let pool = PgPool::connect(&url).await?;
    let f = seed(&pool, "issue875-same-identity").await?;
    ...
    let run = async {
        ...
        assert!(
            desk[0].valid_to.is_some() || desk[0].valid_to_precision.is_some(),
            "after a same-identity update and an explicit reconcile the desk row is still open \
             (reconcile corrected {} rows, recorded {} conflicts): {desk:?}",
